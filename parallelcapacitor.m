function[capac]=parallelcapacitor(wide,thin,dist,volt,eer,refinements)
...
gd=[3 3 3 3;
4 4 4 4;
(-5/2)*wide -wide/2 -wide/2 -wide/2;
(5/2)*wide wide/2 wide/2 wide/2;
(5/2)*wide wide/2 wide/2 wide/2;
(-5/2)*wide -wide/2 -wide/2 -wide/2;
(5/2)*wide thin+(dist/2) -dist/2 dist/2;
(5/2)*wide thin+(dist/2) -dist/2 dist/2;
(-5/2)*wide dist/2 -thin-(dist/2) -dist/2;
(-5/2)*wide dist/2 -thin-(dist/2) -dist/2];
...
work(1,i)=e0*(((gradx(i))^2)+((grady(i))^2))*Ae;
end
end
